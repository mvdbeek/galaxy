...
export default {
    ...
    methods: {
        ...
        selectContent(ids) {
            this.$emit("update:contentSelection", new Set(ids));
        },
        clearSelection() {
            this.selectContent([]);
        },
        ...
        async buildDatasetList() {
            await this._buildNewCollection("list");
        },
        async buildDatasetPair() {
            await this._buildNewCollection("paired");
        },
        async buildListOfPairs() {
            await this._buildNewCollection("list:paired");
        },
        async buildCollectionFromRules() {
            await this._buildNewCollection("rules");
        },
        async _buildNewCollection(collectionTypeCode) {
            const modalResult = await buildCollectionModal(collectionTypeCode, this.history.id, this.selectedContent);
