<template>
    ...
    <StateBtn v-if="inState(STATES.RUNNING)"
        :state="state"
        icon="upload-symbol upload-icon fa fa-spinner fa-spin"
        v-on="$listeners" />

    <StateBtn v-else-if="inState(STATES.QUEUED, STATES.NEW)"
        :state="state"
        icon="fas fa-clock"
        v-on="$listeners" />

    <StateBtn v-else-if="inState(STATES.PAUSED)"
        :state="state"
        icon="fas fa-pause"
        v-on="$listeners" />

    <StateBtn v-else-if="inState(STATES.ERROR)"
        :state="state"
        icon="fas fa-exclamation-triangle"
        v-on="$listeners" />

    <StateBtn v-else-if="inState(STATES.MIXED)"
        :state="state"
