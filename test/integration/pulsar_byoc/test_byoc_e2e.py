...
def _drive_device_flow_with_pair(relay: str, setup) -> dict:
    """Drive RFC 8628 device flow with ``pair=true`` against the live relay
    and return the final token response. Mirrors the production CLI path
    but inlined so we don't shell out to ``pulsar-config``."""
    with httpx.Client(timeout=10.0) as client:
        dev = client.post(
            f"{relay}/auth/device/code",
            data={"client_hint": "byoc-galaxy-e2e", "pair": "true"},
        )
