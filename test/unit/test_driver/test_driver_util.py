...
def _gx_app(new_file_path: str) -> Mock:
    # Every setting caching_fast_app_factory reads has to be spelled out: an unset Mock
    # attribute is a truthy Mock, which would send the factory down the uncached path.
    return Mock(
        config=Mock(
            galaxy_url_prefix="/",
...
def test_tus_routes_are_recognized():
    # Regression guard: FastAPI stores an included router as a single entry carrying no
    # path of its own, so matching on route.path alone silently found nothing here.
...
def test_caching_fast_app_factory_reuses_and_rebinds_across_launches(monkeypatch):
    ...
    # Every test class gets its own temp upload store, so a launch that differs only in
    # that path must still reuse the cached app - rebuilding it was the whole cost.
