...
class FakeConfig:
    ...
    amqp_internal_connection: Optional[str] = None
...
class TestIterToolSources:
    ...
    def test_empty_toolbox_yields_nothing(self):
        """Empty toolbox should yield no results."""
        toolbox = FakeToolbox({})
        result = list(iter_tool_sources(toolbox))
        assert result == []

    def test_yields_tool_id_version_source_dir(self):
        """Each yielded tuple contains (tool_id, version, tool_source, tool_dir)."""
        tool = FakeTool(version="1.0", tool_source=object(), tool_dir="/my/path")
        toolbox = FakeToolbox({"my_tool": tool})

        result = list(iter_tool_sources(toolbox))

        assert len(result) == 1
        tool_id, version, source, tool_dir = result[0]
        assert tool_id == "my_tool"
        assert version == "1.0"
        assert source is tool.tool_source
        assert tool_dir == "/my/path"
    ...
    def test_skips_tools_without_tool_source(self):
        """Tools with no tool_source attribute should be skipped."""
