...
class MappingTests(BaseModelTestCase):
    ...
    def _setup_collection_with_elements(self, n_elements=100, flush=True):
        model = self.model
        ...
        c = model.DatasetCollection(collection_type="list")
        dces = [model.DatasetCollectionElement(collection=c, element=d1, element_identifier=str(i), element_index=i) for i in range(n_elements)]
        self.persist(u, h1, d1, c, *dces, flush=False, expunge=False)
        if flush:
            model.session.flush()
        return c, dces

    def test_collection_get_interface_integer(self):
        n_elements = 100
        c, dces = self._setup_collection_with_elements(n_elements=n_elements)
        for i in range(n_elements):
            assert c[i] == dces[i]
        with pytest.raises(KeyError):
            c[n_elements]

    def test_collection_get_interface_by_identifier(self):
        n_elements = 100
        c, dces = self._setup_collection_with_elements(n_elements=n_elements)
        for i in range(n_elements):
            assert c[str(i)] == dces[i]
        with pytest.raises(KeyError):
            c[str(n_elements)]

    def test_collection_get_interface_by_identifier_in_memory(self):
        n_elements = 100
        c, dces = self._setup_collection_with_elements(n_elements=n_elements, flush=False)
        for i in range(n_elements):
            assert c[str(i)] == dces[i]
        with pytest.raises(KeyError):
            c[str(n_elements)]
