...
def test_discover_datasets_for_nested_collection_with_flat_pattern_raises(mocker):
    """Test that discovering datasets with insufficient identifier levels for a
    nested collection type (e.g. list:paired) raises a clear error instead of
    an opaque AssertionError in build_elements."""
    app = _mock_app()
    sa_session = app.model.context
    ...
    input_dbkey = "?"
    final_job_state = JobState.OK
    # __name__ pattern only produces a single identifier level (just "name"),
    # but list:paired requires two levels (list identifier + paired identifier)
    collection_description = FilePatternDatasetCollectionDescription(pattern="__name__")
    collection = model.DatasetCollection(collection_type="list:paired", populated=False)
    sa_session.add(collection)
    job_context = JobContext(
        tool,
        tool_provided_metadata,
    ...
    filenames = job_context.find_files(output_name, collection, dataset_collectors)
    assert len(filenames) == 10
    with pytest.raises(Exception, match="Cannot add dataset.*directly to a collection.*list:paired.*identifier levels"):
        job_context.populate_collection_elements(
            collection,
            collection_builder,
            filenames,
