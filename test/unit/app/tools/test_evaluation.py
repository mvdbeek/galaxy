...
class ToolEvaluatorTestCase(TestCase, UsesApp):
    def setUp(self):
        ...
        self.evaluator = ToolEvaluator(
            self.app, self.tool, self.job, self.test_directory
        )
    ...
    def test_simple_evaluation(self):
        ...
        command_line, version_command_line, extra_filenames, _ = self.evaluator.build()
        self.assertEqual(
            command_line,
            "bwa --thresh=4 --in=/galaxy/files/dataset_1.dat --out=/galaxy/files/dataset_2.dat",
        )
    ...
    def test_repeat_evaluation(self):
        ...
        self.job.parameters = [
            JobParameter(
                name="r",
                value="""[{"thresh": 4, "__index__": 0},{"thresh": 5, "__index__": 1}]""",
            )
        ]
    ...
    def test_conditional_evaluation(self):
        ...
        self.job.parameters = [
            JobParameter(
                name="c",
                value="""{"thresh": 4, "always_true": "true", "__current_case__": 0}""",
            )
        ]
        self.tool._command_line = (
            "prog1 --thresh=${c.thresh} --test_param=${c.always_true}"
        )
        self._set_compute_environment()
        command_line, version_command_line, extra_filenames, _ = self.evaluator.build()
    ...
    def __test_evaluation_with_path_rewrites(self):
        ...
        self._set_compute_environment(
            input_paths=[
                DatasetPath(1, "/galaxy/files/dataset_1.dat", false_path=job_path_1)
            ],
            output_paths=[
                DatasetPath(2, "/galaxy/files/dataset_2.dat", false_path=job_path_2)
            ],
        )
        command_line, version_command_line, extra_filenames, _ = self.evaluator.build()
        self.assertEqual(
            command_line, f"bwa --thresh=4 --in={job_path_1} --out={job_path_2}"
        )
    ...
    def test_configfiles_evaluation(self):
        ...
        self.assertEqual(
            os.path.join(self.test_directory, "configs", config_basename),
            config_filename,
        )
    ...
    def test_template_property_app(self):
        self._assert_template_property_is(
            "$__app__.config.new_file_path", self.app.config.new_file_path
        )

    def test_template_property_new_file_path(self):
        self._assert_template_property_is(
            "$__new_file_path__", self.app.config.new_file_path
        )
    ...
    def _assert_template_property_is(self, expression, value):
        ...
        self._set_compute_environment()
        _, _, extra_filenames, _ = self.evaluator.build()
        config_filename = extra_filenames[0]
    ...
    def _setup_test_bwa_job(self):
        ...
        self.job.input_datasets = [
            JobToInputDatasetAssociation(name=name, dataset=hda(id, name, path))
        ]

        id, name, path = 112, "output1", "/galaxy/files/dataset_2.dat"
        self.job.output_datasets = [
            JobToOutputDatasetAssociation(name=name, dataset=hda(id, name, path))
        ]
