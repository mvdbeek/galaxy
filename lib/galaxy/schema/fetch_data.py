...
class SecondaryFileSrc(str, Enum):
    """Source types for secondary files (subset of main Src)."""
...
class SecondaryFileSpec(FetchBaseModel):
    ...
    Secondary files are associated files like BAM indexes (.bai) that can be imported
    instead of being regenerated by Galaxy during metadata setting.
    """

    src: SecondaryFileSrc = Field(..., description="Source type for the secondary file")
    path: Optional[str] = Field(None, description="Path to the secondary file (for path/server_dir sources)")
    url: Optional[str] = Field(None, description="URL to fetch the secondary file from")
    ftp_path: Optional[str] = Field(None, description="FTP path to the secondary file")
    metadata_key: str = Field(
        ...,
        description="Metadata key to store this file (e.g., 'bam_index', 'tabix_index')",
    )
...
class BaseDataElement(FetchBaseModel):
    ...
    secondary_files: Optional[list[SecondaryFileSpec]] = Field(
        None,
        description="Pre-existing secondary files (e.g., indexes) to import alongside the primary data. "
        "When provided, Galaxy will use these instead of regenerating them during metadata setting.",
    )
    model_config = ConfigDict(extra="forbid")
