...
class Bam( Tabular, Binary ):
    ...
    column_types = column_types = ['str', 'int', 'str', 'int', 'int', 'str', 'str', 'int', 'int', 'str', 'str', 'str']
    ...
    def set_meta( self, dataset, overwrite=True, **kwd ):
        ...
        try:
            ...
            dataset.metadata.data_lines = bam_file.mappped
    ...
    def display_data(self, trans, dataset, preview=False, filename=None, to_ext=None, **kwd):
        ...
        if preview:
            ...
            return Binary.display_data(self, trans, dataset, preview, filename, to_ext, **kwd)

    def get_chunk(self, trans, dataset, offset=0, ck_size=None):
        start = offset
        end = start + 100
        lines_it = self.line_dataprovider(dataset)
        lines = []
        log.debug("Start %s, end %s" % (start, end))
        for i, line in enumerate(lines_it):
             if start < i < end:  # replace with proper line demand
                 fields = line.split('\t')
                 lines.append("%s\t%s" % ("\t".join(fields[:11]), " ".join(fields[11:])))
        return dumps( { 'ck_data': util.unicodify("\n".join(lines)), 'ck_index': end } )
...
class ConnectivityTable( Tabular ):
    ...
    def get_chunk(self, trans, dataset, chunk):
        ...
        log.debug(dumps( { 'ck_data': util.unicodify(ck_data_header + "\n" + ck_data_body ), 'ck_index': ck_index + 1 } ))
