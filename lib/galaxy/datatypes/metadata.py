...
def _flag_modified_if_mapped(instance, key):
...
class MetadataCollection(Mapping):
    ...
    def from_JSON_dict(self, filename=None, path_rewriter=None, json_dict=None):
        ...
        _flag_modified_if_mapped(dataset, "_metadata")
...
class FileParameter(MetadataParameter):
    ...
    def wrap(self, value, session):
        ...
        if isinstance(value, MetadataTempFile):
            return value
        metadata_file_class = _metadata_file_class()
        if isinstance(value, metadata_file_class):
            return value
        if isinstance(value, int):
            return session.get(metadata_file_class, value)
        else:
            wrapped_value = session.execute(select(metadata_file_class).filter_by(uuid=value)).scalar_one_or_none()
            if wrapped_value:
                ...
                session.flush()
            return session.execute(select(metadata_file_class).filter_by(uuid=value)).scalar_one_or_none()
    ...
    def make_copy(self, value, target_context: MetadataCollection, source_context):
        ...
        if value and target_dataset.object_store.exists(target_dataset):
            ...
            new_value = _metadata_file_class()(dataset=target_context.parent, name=self.spec.name)
    ...
    @classmethod
    def marshal(cls, value):
        if value is None or isinstance(value, (int, str, MetadataTempFile)):
            return value
        if isinstance(value, _metadata_file_class()):
    ...
    def to_external_value(self, value):
        ...
        elif isinstance(value, _metadata_file_class()):
            value = value.id
        return value
