...
model.JobToInputDatasetCollectionElementAssociation.table = Table(
    ...
    Column(
        ...
        Integer,
        ForeignKey("dataset_collection_element.id"),
        index=True,
    ),
    Column("name", Unicode(255)),
)
...
model.WorkflowInvocationOutputDatasetCollectionAssociation.table = Table(
    ...
    Column(
        "workflow_invocation_id",
        Integer,
        ForeignKey("workflow_invocation.id", name="fk_wiodca_wii"),
        index=True,
    ),
    Column(
        "workflow_step_id",
        Integer,
        ForeignKey("workflow_step.id", name="fk_wiodca_wsi"),
        index=True,
    ),
    Column(
        "dataset_collection_id",
    ...
    Column(
        "workflow_output_id",
        Integer,
        ForeignKey("workflow_output.id", name="fk_wiodca_woi"),
        index=True,
    ),
)
...
model.WorkflowInvocationStepOutputDatasetCollectionAssociation.table = Table(
    ...
    Column(
        "workflow_step_id",
        Integer,
        ForeignKey("workflow_step.id", name="fk_wisodca_wsi"),
        index=True,
    ),
    Column(
        "dataset_collection_id",
...
model.WorkflowInvocationToSubworkflowInvocationAssociation.table = Table(
    ...
    Column(
        "workflow_invocation_id",
        Integer,
        ForeignKey("workflow_invocation.id", name="fk_wfi_swi_wfi"),
        index=True,
    ),
    Column(
        "subworkflow_invocation_id",
        Integer,
        ForeignKey("workflow_invocation.id", name="fk_wfi_swi_swi"),
        index=True,
    ),
    Column(
        "workflow_step_id",
        Integer,
        ForeignKey("workflow_step.id", name="fk_wfi_swi_ws"),
    ),
)
...
simple_mapping(
    model.Dataset,
    job=relation(
        model.Job, primaryjoin=(model.Dataset.table.c.job_id == model.Job.table.c.id)
    ),
...
mapper(
    ...
    properties=dict(
        ...
        update_time=column_property(
            select([func.max(model.HistoryAudit.table.c.update_time)]).where(
                model.HistoryAudit.table.c.history_id == model.History.table.c.id
            ),
        ),
    ),
)
...
mapper(
    ...
    properties=dict(
        histories=relation(
            model.History, backref="user", order_by=desc(model.History.update_time)
        ),
    ...
    properties=dict(
        ...
        datasets=relation(
            ...
            primaryjoin=(
                ...
                & model.LibraryDataset.table.c.library_dataset_dataset_association_id.isnot(
                    None
                )
            ),
...
mapper(
    ...
    model.JobToInputDatasetCollectionElementAssociation.table,
    properties=dict(
        job=relation(model.Job),
        dataset_collection_element=relation(model.DatasetCollectionElement, lazy=False),
    ),
)
...
mapper(
    model.InteractiveToolEntryPoint,
    model.InteractiveToolEntryPoint.table,
    properties=dict(
        job=relation(
            model.Job,
            backref=backref("interactivetool_entry_points", uselist=True),
            uselist=False,
        )
    ),
)
...
mapper(
    model.JobContainerAssociation,
    model.JobContainerAssociation.table,
    properties=dict(
        job=relation(
            model.Job, backref=backref("container", uselist=False), uselist=False
        )
    ),
)
...
simple_mapping(
    ...
    subworkflow_invocation_id=column_property(
        select(
            ...
            and_(
                model.WorkflowInvocationToSubworkflowInvocationAssociation.table.c.workflow_invocation_id
                == model.WorkflowInvocationStep.table.c.workflow_invocation_id,
                model.WorkflowInvocationToSubworkflowInvocationAssociation.table.c.workflow_step_id
                == model.WorkflowInvocationStep.table.c.workflow_step_id,
            )
        ),
    ),
)
...
simple_mapping(
    ...
    workflow_invocation_step=relation(
        ...
        primaryjoin=and_(
            model.WorkflowInvocationStep.table.c.workflow_invocation_id
            == model.WorkflowInvocationOutputValue.table.c.workflow_invocation_id,
            model.WorkflowInvocationStep.table.c.workflow_step_id
            == model.WorkflowInvocationOutputValue.table.c.workflow_step_id,
        ),
...
mapper(
    ...
    properties=dict(
        ...
        input_dataset_collection_elements=relation(
            model.JobToInputDatasetCollectionElementAssociation, lazy=True
        ),
...
def db_next_hid(self, n=1):
    ...
    try:
        if "postgres" not in session.bind.dialect.name:
            next_hid = (
                select([table.c.hid_counter], table.c.id == model.cached_id(self))
                .with_for_update()
                .scalar()
            )
