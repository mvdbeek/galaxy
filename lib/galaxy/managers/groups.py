...
class GroupsManager:
    ...
    def show(self, trans: ProvidesAppContext, group_id: DecodedDatabaseIdField):
        """
    ...
    def _check_duplicated_group_name(
        self, trans: ProvidesAppContext, group_name: str
    ) -> None:
        if (
            trans.sa_session.query(model.Group)
            .filter(model.Group.name == group_name)
            .first()
        ):
    ...
    def _get_users_by_ids(
        self, trans: ProvidesAppContext, user_ids: List[DecodedDatabaseIdField]
    ) -> List[model.User]:
        users = (
            trans.sa_session.query(model.User)
            .filter(model.User.table.c.id.in_(user_ids))
            .all()
        )
        return users

    def _get_roles_by_ids(
        self, trans: ProvidesAppContext, role_ids: List[DecodedDatabaseIdField]
    ) -> List[model.Role]:
        roles = (
            trans.sa_session.query(model.Role).filter(model.Role.id.in_(role_ids)).all()
        )
        return roles
