...
from galaxy.schema.remote_files import (
    ...
    DetectedSecondaryFilesResponse,
...
class RemoteFilesManager:
    ...
    def detect_secondary_files(
        ...
        """
        Detect secondary files (e.g., indexes) that exist alongside a primary file.

        This method looks for known secondary files like .bai for BAM files
        that can be imported instead of being regenerated by Galaxy.

        Args:
            user_ctx: The user context for file access permissions
            target: URI or path to the primary file
            extension: Optional file extension hint (e.g., 'bam')

        Returns:
            DetectedSecondaryFilesResponse containing list of detected secondary files
        """
        from galaxy.datatypes.index_patterns import detect_secondary_files as detect_patterns

        user_file_source_context = ProvidesFileSourcesUserContext(user_ctx)

        # Parse the target URI to get the file source and path
        if "://" in target:
            self._file_sources.validate_uri_root(target, user_context=user_file_source_context)
            file_source_path = self._file_sources.get_file_source_path(target)
            file_source = file_source_path.file_source
            path = file_source_path.path

            # Get the real path for detection
            try:
                real_path = file_source.to_dict(path, user_context=user_file_source_context).get("path", path)
            except Exception:
                real_path = path
        else:
            real_path = target

        # Detect secondary files using the pattern registry
        detected = detect_patterns(real_path, extension)

        # Convert to response model
        result = []
        for det in detected:
            # Build the URI for the secondary file if original was a URI
            if "://" in target:
                # Construct URI by replacing the primary file path with the secondary path
                secondary_uri = target.rsplit("/", 1)[0] + "/" + det.path.rsplit("/", 1)[-1]
            else:
                secondary_uri = det.path

            result.append(
                DetectedSecondaryFile(
                    path=secondary_uri,
                    metadata_key=det.metadata_key,
                    description=det.description,
                    suggested=True,
                )
            )

        return DetectedSecondaryFilesResponse(result)
