...
class HistoryExportView:
    ...
    def serialize(self, trans, history_id: int, jeha):
        ...
        api_url = trans.url_builder(
            "history_archive_download", id=encoded_history_id, jeha_id=encoded_jeha_id
        )
    ...
    def get_ready_jeha(
        self, trans, history_id, jeha_id: Union[int, LatestLiteral] = "latest"
    ):
        history = self._history(trans, history_id)
        matching_exports = history.exports
        if jeha_id != "latest":
