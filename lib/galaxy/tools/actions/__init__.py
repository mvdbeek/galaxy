                    ...
                    for i, v in enumerate(value):
                        ...
                        for (
                            ...
                            if not new_data or new_data.datatype.matches_any(
                                ...
                                raise Exception(
                                    "A path for explicit datatype conversion has not been found: {} --/--> {}".format(
                                        input_datasets[
                                            prefix + input.name + str(i + 1)
                                        ].extension,
                                        conversion_extensions,
...
class OutputCollections:
    ...
    def create_collection(
        self, output, name, collection_type=None, completed_job=None, **element_kwds
    ):
...
def filter_output(tool, output, incoming):
    for filter in output.filters:
        try:
            ...
            log.debug(
                "Tool %s output %s: dataset output filter (%s) failed: %s"
                % (tool.id, output.name, filter.text, e)
            )
    return False
