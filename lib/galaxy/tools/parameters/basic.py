    ...
    def to_param_dict_string(self, value, other_values=None):
        if self.rgb:
            try:
                ...
                raise ParameterValueError(
                    "Failed to convert '%s' to RGB." % value, self.name
                )
        return str(value)
    ...
    def from_json(self, value, trans, other_values=None, require_legal_value=True):
        ...
        if isinstance(value, list):
            ...
            if set(value).issubset(legal_values):
                ...
                raise ParameterValueError(
                    "invalid options ({!r}) were selected (valid options: {})".format(
                        ",".join(set(value) - set(legal_values)), ",".join(legal_values)
                    ),
                    self.name,
                    is_dynamic=self.is_dynamic,
                )
            ...
            if value in legal_values:
                ...
                raise ParameterValueError(
                    "an invalid option ({!r}) was selected (valid options: {})".format(
                        value, ",".join(legal_values)
                    ),
                    self.name,
                    value,
                    is_dynamic=self.is_dynamic,
...
class ColumnListParameter(SelectToolParameter):
    ...
    def get_column_list(self, trans, other_values):
        ...
        for dataset in util.listify(dataset):
            ...
            if (
                isinstance(dataset, trans.app.model.HistoryDatasetAssociation)
                and self.ref_input
...
class DataCollectionToolParameter(BaseDataToolParameter):
    ...
    def match_collections(self, trans, history, dataset_collection_matcher):
        dataset_collections = (
            trans.app.dataset_collections_service.history_dataset_collections(
    ...
    def to_dict(self, trans, other_values=None):
        ...
        # append DCE
        if isinstance(
            other_values.get(self.name), galaxy.model.DatasetCollectionElement
        ):
