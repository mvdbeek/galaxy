...
class BaseDataToolParameter(ToolParameter):
    ...
    def _paginated_visible_datasets(self, trans, history, *, extensions, valid_states, search=None, offset=0, limit=50):
        ...
        cached = trans.get_cache_value(key, _PAGE_CACHE_MISS) if hasattr(trans, "get_cache_value") else _PAGE_CACHE_MISS
        ...
        if hasattr(trans, "set_cache_value"):
            trans.set_cache_value(key, result)
        return result

    def _paginated_dataset_collections(self, trans, history, *, visible_only, search=None, offset=0, limit=50):
    ...
    def get_initial_value(self, trans, other_values):
        ...
        if (history := trans.history) is not None:
            ...
            if isinstance(self, DataToolParameter):
                ...
                db_offset = 0
                while True:
                    rows, total = self._paginated_dataset_collections(
                        trans,
                        history,
                        visible_only=True,
                        offset=db_offset,
