...
class DataMetaFilter(Filter):
    ...
    def filter_options(self, options, trans, other_values):
        ...
        try:
            ...
            log.warning(
                "could not filter by metadata: %s not a data or collection parameter"
                % self.ref_name
            )
            return []
    ...
    def get_fields(self, trans, other_values):
        if self.dataset_ref_name:
            try:
                ...
                log.warning(
                    "could not create dynamic options from_dataset: %s unknown"
                    % self.dataset_ref_name
                )
                return []
            except ValueError:  # not a valid dataset
                log.warning(
                    "could not create dynamic options from_dataset: %s not a data or collection parameter"
                    % self.dataset_ref_name
                )
                return []
