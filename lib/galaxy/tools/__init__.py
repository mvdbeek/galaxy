...
class HarmonizeTool(DatabaseOperationTool):
    ...
    def _produce_outputs_with_optional_nulls(self, trans, output_collections, hdca1, elements1, history):
        ...
        for element in elements1:
            ...
            if getattr(dce_object, "history_content_type", None) == "dataset":
                ...
                null_datasets.append(null_hda)
            else:
                # Sub-collection (e.g. paired) — create matching structure with null datasets
                new_elements2[identifier] = self._create_null_subcollection(
                    trans, history, dce_object, object_store_populator, null_datasets
                )
        self._add_datasets_to_history(history, null_datasets)
        output_collections.create_collection(
    ...
    def _create_null_subcollection(self, trans, history, source_collection, object_store_populator, null_datasets):
        ...
        for dce in source_collection.elements:
            ...
            if getattr(dce_object, "history_content_type", None) == "dataset":
                ...
                sub_elements[dce.element_identifier] = self._create_null_subcollection(
                    trans, history, dce_object, object_store_populator, null_datasets
                )
