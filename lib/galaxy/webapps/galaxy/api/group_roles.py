...
GroupIDParam: DecodedDatabaseIdField = Path(
    ..., title="GroupID", description="The ID of the group"
)

RoleIDParam: DecodedDatabaseIdField = Path(
    ..., title="RoleID", description="The ID of the role"
)
...
def group_role_to_model(trans, encoded_group_id, role):
    ...
    return GroupRoleModelResponse(id=encoded_role_id, name=role.name, url=url)
...
class FastAPIGroupRoles:
    ...
    def index(
        ...
        return GroupRoleListModelResponse(
            __root__=[
                group_role_to_model(trans, group_id, gr.role) for gr in group_roles
            ]
        )
