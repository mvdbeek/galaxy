...
class FastAPIRemoteFiles:
    ...
    @router.get(
        "/api/remote_files/secondary_files",
        summary="Detect secondary files (e.g., indexes) for a remote file.",
        response_description="A list of detected secondary files that can be imported alongside the primary file.",
    )
    def detect_secondary_files(
        ...
        target: Annotated[
            ...
            Query(
                title="Target file",
                description="The URI or path of the primary file to detect secondary files for.",
            ),
        ] = ...,
        extension: Annotated[
            Optional[str],
            Query(
                ...
                description="Optional file extension hint (e.g., 'bam', 'vcf_bgzip'). "
                "If not provided, will be inferred from the file path.",
            ),
        ] = None,
    ) -> DetectedSecondaryFilesResponse:
        """
        Detect secondary files (e.g., BAM indexes) that exist alongside a primary file.

        This endpoint allows the frontend to discover pre-existing secondary files
        (such as .bai index files for BAM files) that can be imported instead of
        being regenerated by Galaxy during metadata setting.

        The detected files can then be specified in the `secondary_files` parameter
        of a fetch/upload request to skip regeneration.
        """
        return self.manager.detect_secondary_files(user_ctx, target, extension)
