...
class HistoriesController(
    BaseGalaxyAPIController, ExportsHistoryMixin, ImportsHistoryMixin
):
    ...
    history_export_view: histories.HistoryExportView = depends(
        histories.HistoryExportView
    )
    ...
    def create(self, trans, payload, **kwd):
        ...
        * POST /api/histories:
        ...
        if "archive_source" in payload:
            ...
            job = self.queue_history_import(
                trans, archive_type=archive_type, archive_source=archive_source
            )
            job_dict = job.to_dict()
            job_dict["message"] = (
                "Importing history from source '%s'. This history will be visible when the import is complete."
                % archive_source
            )
