...
class JobWrapper(HasResourceParameters):
    ...
    def prepare(self, compute_environment=None):
        ...
        if version_string_cmd_raw:
            ...
            self.write_version_cmd = (
                f"{version_string_cmd} > {compute_environment.version_path()} 2>&1"
            )
        else:
            self.write_version_cmd = None
    ...
    def finish(
        ...
        if (
            ...
            and outputs_to_working_directory
            and not self.__link_file_check()
        ):
    ...
    def get_output_path(self, dataset):
        ...
        ), f"{dataset} needs to be flushed to find output path"
        if self.output_paths is None:
            self.compute_outputs()
        for (hda, dataset_path) in self.output_hdas_and_paths.values():
            if hda.id == dataset.id:
                return dataset_path
        raise KeyError(
            f"Couldn't find job output for [{dataset}] in [{self.output_hdas_and_paths.values()}]"
        )
...
class SharedComputeEnvironment(SimpleComputeEnvironment):
    ...
    def galaxy_url(self):
        return self.job_wrapper.get_destination_configuration(
