...
class ToolProxy(metaclass=ABCMeta):
    ...
    def to_persistent_representation(self):
        ...
        return {
            "class": self._class,
            "raw_process_reference": json.dumps(persisted_obj),
            "uuid": self._uuid,
        }
    ...
    def from_persistent_representation(as_object, strict_cwl_validation=True, tool_directory=None):
        ...
        loaded_object = tool_proxy(tool_object=json.loads(as_object['raw_process_reference']), strict_cwl_validation=strict_cwl_validation, tool_directory=tool_directory, uuid=as_object.get('uuid'))
