                    ...
                    if entry["class"] == "File":
                        if "contents" in entry:
                            ...
                            entry_path = os.path.join(
                                test_data_directory, entry["path"]
                            )
                            os.symlink(
                                entry_path, str((base_dir / entry["path"]).resolve())
                            )
                    else:
    ...
    if galaxy_output.history_content_type == "raw_value":
        ...
        if ext == "expression.json":
            ...
            if file_or_directory == "File":
                dataset_dict = get_dataset(output_metadata)
                properties = output_properties(pseduo_location=pseduo_location, cwl_formats=output_metadata['cwl_formats'], **dataset_dict)
                ...
                _handle_pseudo_location(properties, pseduo_location=pseduo_location)
