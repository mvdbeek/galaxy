...
class TestDisplayApplicationsApi(ApiTestCase):
    ...
    def test_chained_slow_conversion_no_duplicates(self):
        ...
        This test uses slow converters (with 10-second delays) to reproduce the race condition
        ...
        Setup:
        - interval datatype has display app that requests slow_tabular format
        - interval -> slow_tabular converter has depends_on="slow_bed"
        - So conversion chain is: interval -> slow_bed -> slow_tabular

        Test flow:
        1. Upload interval file
        2. Make multiple rapid requests to the display app (triggers chained conversion)
        3. Check that only one slow_bed and one slow_tabular conversion are created
        """
        # Upload interval file - display app on interval requests slow_tabular
        # which triggers the chained conversion via depends_on
        test_file = self.get_filename("1.interval")
        with self.dataset_populator.test_history() as history_id:
            # Upload interval file
            interval_ds = self.dataset_populator.new_dataset(
                history_id,
                content=open(test_file, "rb"),
                file_type="interval",
                wait=True,
            ...
            # Make multiple rapid requests to the display app on interval
            # This triggers: interval -> slow_bed -> slow_tabular chain
            # The depends_on="slow_bed" causes the chained conversion
            payload = {
                "app_name": "test_slow_conversion",
                "dataset_id": interval_ds["id"],
                "link_name": "local_default",
            }

            # Make multiple rapid requests during the slow conversion (10 seconds)
            # Without the fix, each request might not see the in-progress conversion
            # and start a new one
            for i in range(10):
                response = self._post("display_applications/create_link", data=payload, json=True)
                self._assert_status_code_is(response, 200)
            ...
            # Check for slow_bed conversions (intermediate step in chain)
            slow_bed_datasets = [
            ...
            # Check for slow_tabular conversions (final step in chain)
            ...
            # Without the fix, we would see multiple datasets due to the
            # implicitly_converted_datasets relationship cache not seeing
            # the newly created ICDA
