...
class LibrariesApiTestCase(ApiTestCase, TestsDatasets):
    ...
    def test_create_datasets_in_library_from_collection(self):
        ...
        folder_id = folder_response.json()[0]['id']
        history_id = self.dataset_populator.new_history()
        hdca_id = self.dataset_collection_populator.create_list_in_history(history_id, contents=["xxx", "yyy"], direct_upload=True).json()["output_collections"][0]["id"]
    ...
    def test_create_datasets_in_folder_from_collection(self):
        ...
        # is refused
        hdca_pair_id = self.dataset_collection_populator.create_list_of_pairs_in_history(history_id).json()["output_collections"][0]['id']
