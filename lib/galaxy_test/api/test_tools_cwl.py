...
class TestCwlTools(ApiTestCase):
    ...
    def test_galactic_record_input(self):
        with self.dataset_populator.test_history() as history_id:
            ...
            hda2_id = self.dataset_populator.new_dataset(history_id, content="cow dog foo", name="test_dataset.txt")[
                "id"
            ]
            self.dataset_populator.wait_for_history(history_id, assert_ok=True)
            inputs = {
            ...
            run_response = self._run(
                "galactic_record_input.cwl#galactic_record_input", history_id, inputs, assert_ok=True
            )
            dataset = run_response["outputs"][0]
            content = self.dataset_populator.get_history_dataset_content(history_id, dataset=dataset)
