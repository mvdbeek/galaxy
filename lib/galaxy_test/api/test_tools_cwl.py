...
class CwlToolsTestCase(ApiTestCase):
    ...
    def test_cat1_number_cwl_json_file(self):
        ...
        run_object = self.cwl_populator.run_cwl_job("cat1-tool.cwl", "test/functional/tools/cwl_tools/v1.0_custom/cat-job.json")
        stdout = self._get_job_stdout(run_object.job_id)
        self.assertEqual(stdout, "Hello world!\n")

    @skip_without_tool("cat1-tool.cwl")
    def test_cat1_number_cwl_n_json_file(self):
        run_object = self.cwl_populator.run_cwl_job("cat1-tool.cwl", "test/functional/tools/cwl_tools/v1.0_custom/cat-n-job.json")
        stdout = self._get_job_stdout(run_object.job_id)
    ...
    def test_null_expression_any_bad_1(self):
        ...
        run_object = self.cwl_populator.run_cwl_job("null-expression2-tool.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/null-expression1-job.json", assert_ok=False)
    ...
    def test_null_expression_any_bad_2(self):
        ...
        run_object = self.cwl_populator.run_cwl_job("null-expression2-tool.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/empty.json", assert_ok=False)
        with self.assertRaises(AssertionError):
            run_object.wait()
    ...
    def test_parse_int_tool(self):
        run_object = self.cwl_populator.run_cwl_job("parseInt-tool.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/parseInt-job.json")
    ...
    def test_record_output(self):
        run_object = self.cwl_populator.run_cwl_job("record-output.cwl", "test/functional/tools/cwl_tools/v1.0/v1.0/record-output-job.json")
