...
class TestToolsUpload(ApiTestCase):
    ...
    def test_stage_fetch_decompress_true(self, history_id: str, mock_http_server) -> None:
        url = mock_http_server.get_url(
            remote_url="https://github.com/galaxyproject/galaxy/blob/dev/test-data/1.fasta.gz?raw=true",
            file_path="test-data/1.fasta.gz",
        )
        job = {
            "input1": {
                "class": "File",
    ...
    def test_stage_fetch_decompress_false(self, history_id: str, mock_http_server) -> None:
        url = mock_http_server.get_url(
            remote_url="https://github.com/galaxyproject/galaxy/blob/dev/test-data/1.fasta.gz?raw=true",
            file_path="test-data/1.fasta.gz",
        )
