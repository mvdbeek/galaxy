...
__all__ = ["BaseUrlParameterModel"]


@dataclass
class BaseUrlParameterModel:
    """
    BaseUrlParameterModel dataclass
    ...
    Args:
        ...
        type_ (GalaxyToolParameterModelOutputTypeEnum)
                                 : Maps from 'type'
        argument (BaseUrlParameterModelArgument | None)
        ...
        help_ (Help15 | None)    : Short bit of text, rendered on the tool form just below
                                   the associated field to provide information about the
                                   field. (maps from 'help')
        hidden (bool | None)     :
        is_dynamic (bool | None) :
        label (BaseUrlParameterModelLabel | None)
    ...
    label: BaseUrlParameterModelLabel | None = None  # Will be displayed on the tool page as the label of the parameter.
    optional: bool | None = False  # If `false`, parameter must have a value.
    parameter_type: str | None = "gx_baseurl"
