...
__all__ = ["ParsedWorkbookForCollectionRowsItem"]


@dataclass
class ParsedWorkbookForCollectionRowsItem:
    """
    ...
    Example:
        ...
        obj = structure_from_dict({"key": {"field": "value"}}, ParsedWorkbookForCollectionRowsItem)
...
# Register cattrs hooks for ParsedWorkbookForCollectionRowsItem
def _structure_parsedworkbookforcollectionrowsitem(
    data: dict[str, Any], _: type[ParsedWorkbookForCollectionRowsItem]
) -> ParsedWorkbookForCollectionRowsItem:
    """Structure hook for cattrs to handle ParsedWorkbookForCollectionRowsItem deserialization with typed values."""
    if data is None:
        return ParsedWorkbookForCollectionRowsItem()
    if isinstance(data, ParsedWorkbookForCollectionRowsItem):
        return data
    ...
    return ParsedWorkbookForCollectionRowsItem(_data=structured_data)


def _unstructure_parsedworkbookforcollectionrowsitem(instance: ParsedWorkbookForCollectionRowsItem) -> dict[str, Any]:
    """Unstructure hook for cattrs to handle ParsedWorkbookForCollectionRowsItem serialization."""
...
converter.register_structure_hook(ParsedWorkbookForCollectionRowsItem, _structure_parsedworkbookforcollectionrowsitem)
converter.register_unstructure_hook(
    ParsedWorkbookForCollectionRowsItem, _unstructure_parsedworkbookforcollectionrowsitem
)
