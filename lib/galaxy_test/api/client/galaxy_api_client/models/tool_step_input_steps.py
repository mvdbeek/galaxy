...
__all__ = ["ToolStepInputSteps"]


@dataclass
class ToolStepInputSteps:
    """
    ...
    Example:
        ...
        obj = structure_from_dict({"key": {"field": "value"}}, ToolStepInputSteps)
...
# Register cattrs hooks for ToolStepInputSteps
def _structure_toolstepinputsteps(data: dict[str, Any], _: type[ToolStepInputSteps]) -> ToolStepInputSteps:
    """Structure hook for cattrs to handle ToolStepInputSteps deserialization with typed values."""
    if data is None:
        return ToolStepInputSteps()
    if isinstance(data, ToolStepInputSteps):
        return data
    ...
    return ToolStepInputSteps(_data=structured_data)


def _unstructure_toolstepinputsteps(instance: ToolStepInputSteps) -> dict[str, Any]:
    """Unstructure hook for cattrs to handle ToolStepInputSteps serialization."""
...
converter.register_structure_hook(ToolStepInputSteps, _structure_toolstepinputsteps)
converter.register_unstructure_hook(ToolStepInputSteps, _unstructure_toolstepinputsteps)
