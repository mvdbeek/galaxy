...
__all__ = ["PauseStepInputSteps"]


@dataclass
class PauseStepInputSteps:
    """
    ...
    Example:
        ...
        obj = structure_from_dict({"key": {"field": "value"}}, PauseStepInputSteps)
...
# Register cattrs hooks for PauseStepInputSteps
def _structure_pausestepinputsteps(data: dict[str, Any], _: type[PauseStepInputSteps]) -> PauseStepInputSteps:
    """Structure hook for cattrs to handle PauseStepInputSteps deserialization with typed values."""
    if data is None:
        return PauseStepInputSteps()
    if isinstance(data, PauseStepInputSteps):
        return data
    ...
    return PauseStepInputSteps(_data=structured_data)


def _unstructure_pausestepinputsteps(instance: PauseStepInputSteps) -> dict[str, Any]:
    """Unstructure hook for cattrs to handle PauseStepInputSteps serialization."""
...
converter.register_structure_hook(PauseStepInputSteps, _structure_pausestepinputsteps)
converter.register_unstructure_hook(PauseStepInputSteps, _unstructure_pausestepinputsteps)
