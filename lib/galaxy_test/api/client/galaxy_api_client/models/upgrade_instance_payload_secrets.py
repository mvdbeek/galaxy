...
__all__ = ["UpgradeInstancePayloadSecrets"]


@dataclass
class UpgradeInstancePayloadSecrets:
    """
    ...
    Example:
        ...
        obj = structure_from_dict({"key": {"field": "value"}}, UpgradeInstancePayloadSecrets)
...
# Register cattrs hooks for UpgradeInstancePayloadSecrets
def _structure_upgradeinstancepayloadsecrets(
    data: dict[str, Any], _: type[UpgradeInstancePayloadSecrets]
) -> UpgradeInstancePayloadSecrets:
    """Structure hook for cattrs to handle UpgradeInstancePayloadSecrets deserialization with typed values."""
    if data is None:
        return UpgradeInstancePayloadSecrets()
    if isinstance(data, UpgradeInstancePayloadSecrets):
        return data
    ...
    return UpgradeInstancePayloadSecrets(_data=structured_data)


def _unstructure_upgradeinstancepayloadsecrets(instance: UpgradeInstancePayloadSecrets) -> dict[str, Any]:
    """Unstructure hook for cattrs to handle UpgradeInstancePayloadSecrets serialization."""
...
converter.register_structure_hook(UpgradeInstancePayloadSecrets, _structure_upgradeinstancepayloadsecrets)
converter.register_unstructure_hook(UpgradeInstancePayloadSecrets, _unstructure_upgradeinstancepayloadsecrets)
