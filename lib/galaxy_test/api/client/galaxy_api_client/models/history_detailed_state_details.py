...
__all__ = ["HistoryDetailedStateDetails"]


@dataclass
class HistoryDetailedStateDetails:
    """
    ...
    Example:
        ...
        obj = structure_from_dict({"key": {"field": "value"}}, HistoryDetailedStateDetails)
...
# Register cattrs hooks for HistoryDetailedStateDetails
def _structure_historydetailedstatedetails(
    data: dict[str, Any], _: type[HistoryDetailedStateDetails]
) -> HistoryDetailedStateDetails:
    """Structure hook for cattrs to handle HistoryDetailedStateDetails deserialization with typed values."""
    if data is None:
        return HistoryDetailedStateDetails()
    if isinstance(data, HistoryDetailedStateDetails):
        return data
    ...
    return HistoryDetailedStateDetails(_data=structured_data)


def _unstructure_historydetailedstatedetails(instance: HistoryDetailedStateDetails) -> dict[str, Any]:
    """Unstructure hook for cattrs to handle HistoryDetailedStateDetails serialization."""
...
converter.register_structure_hook(HistoryDetailedStateDetails, _structure_historydetailedstatedetails)
converter.register_unstructure_hook(HistoryDetailedStateDetails, _unstructure_historydetailedstatedetails)
