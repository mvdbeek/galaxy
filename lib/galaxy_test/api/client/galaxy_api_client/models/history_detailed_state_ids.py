...
__all__ = ["HistoryDetailedStateIds"]


@dataclass
class HistoryDetailedStateIds:
    """
    ...
    Example:
        ...
        obj = structure_from_dict({"key": {"field": "value"}}, HistoryDetailedStateIds)
...
# Register cattrs hooks for HistoryDetailedStateIds
def _structure_historydetailedstateids(
    data: dict[str, Any], _: type[HistoryDetailedStateIds]
) -> HistoryDetailedStateIds:
    """Structure hook for cattrs to handle HistoryDetailedStateIds deserialization with typed values."""
    if data is None:
        return HistoryDetailedStateIds()
    if isinstance(data, HistoryDetailedStateIds):
        return data
    ...
    return HistoryDetailedStateIds(_data=structured_data)


def _unstructure_historydetailedstateids(instance: HistoryDetailedStateIds) -> dict[str, Any]:
    """Unstructure hook for cattrs to handle HistoryDetailedStateIds serialization."""
...
converter.register_structure_hook(HistoryDetailedStateIds, _structure_historydetailedstateids)
converter.register_unstructure_hook(HistoryDetailedStateIds, _unstructure_historydetailedstateids)
