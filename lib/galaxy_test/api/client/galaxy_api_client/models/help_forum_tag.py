...
class HelpForumTag:
    ...
    Example:
        ...
        obj = structure_from_dict({"key": "value"}, HelpForumTag)
...
# Register cattrs hooks for HelpForumTag
def _structure_helpforumtag(data: dict[str, Any], _: type[HelpForumTag]) -> HelpForumTag:
    """Structure hook for cattrs to handle HelpForumTag deserialization."""
    if data is None:
        return HelpForumTag()
    if isinstance(data, HelpForumTag):
        return data
    return HelpForumTag(_data=data)


def _unstructure_helpforumtag(instance: HelpForumTag) -> dict[str, Any]:
    """Unstructure hook for cattrs to handle HelpForumTag serialization."""
...
converter.register_structure_hook(HelpForumTag, _structure_helpforumtag)
converter.register_unstructure_hook(HelpForumTag, _unstructure_helpforumtag)
