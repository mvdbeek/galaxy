...
class Config_:
    ...
    Example:
        ...
        obj = structure_from_dict({"key": "value"}, Config_)
...
# Register cattrs hooks for Config_
def _structure_config_(data: dict[str, Any], _: type[Config_]) -> Config_:
    """Structure hook for cattrs to handle Config_ deserialization."""
    if data is None:
        return Config_()
    if isinstance(data, Config_):
        return data
    return Config_(_data=data)


def _unstructure_config_(instance: Config_) -> dict[str, Any]:
    """Unstructure hook for cattrs to handle Config_ serialization."""
...
converter.register_structure_hook(Config_, _structure_config_)
converter.register_unstructure_hook(Config_, _unstructure_config_)
