...
__all__ = ["ToolDataFieldFiles"]


@dataclass
class ToolDataFieldFiles:
    """
    ...
    Example:
        ...
        obj = structure_from_dict({"key": {"field": "value"}}, ToolDataFieldFiles)
...
# Register cattrs hooks for ToolDataFieldFiles
def _structure_tooldatafieldfiles(data: dict[str, Any], _: type[ToolDataFieldFiles]) -> ToolDataFieldFiles:
    """Structure hook for cattrs to handle ToolDataFieldFiles deserialization with typed values."""
    if data is None:
        return ToolDataFieldFiles()
    if isinstance(data, ToolDataFieldFiles):
        return data
    ...
    return ToolDataFieldFiles(_data=structured_data)


def _unstructure_tooldatafieldfiles(instance: ToolDataFieldFiles) -> dict[str, Any]:
    """Unstructure hook for cattrs to handle ToolDataFieldFiles serialization."""
...
converter.register_structure_hook(ToolDataFieldFiles, _structure_tooldatafieldfiles)
converter.register_unstructure_hook(ToolDataFieldFiles, _unstructure_tooldatafieldfiles)
