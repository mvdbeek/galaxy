from __future__ import annotations

from collections.abc import Mapping
from typing import TYPE_CHECKING, Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field

if TYPE_CHECKING:
    from ..models.notification_response import NotificationResponse


T = TypeVar("T", bound="NotificationCreatedResponse")


@_attrs_define
class NotificationCreatedResponse:
    """
    Attributes:
        notification (NotificationResponse): Basic common fields for all notification responses.
        total_notifications_sent (int): The total number of notifications that were sent to the recipients.
    """

    notification: NotificationResponse
    total_notifications_sent: int
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        notification = self.notification.to_dict()

        total_notifications_sent = self.total_notifications_sent

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "notification": notification,
                "total_notifications_sent": total_notifications_sent,
            }
        )

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        from ..models.notification_response import NotificationResponse

        d = dict(src_dict)
        notification = NotificationResponse.from_dict(d.pop("notification"))

        total_notifications_sent = d.pop("total_notifications_sent")

        notification_created_response = cls(
            notification=notification,
            total_notifications_sent=total_notifications_sent,
        )

        notification_created_response.additional_properties = d
        return notification_created_response

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
