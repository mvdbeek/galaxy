...
__all__ = ["JobStateSummaryStates"]


@dataclass
class JobStateSummaryStates:
    """
    ...
    Example:
        ...
        obj = structure_from_dict({"key": {"field": "value"}}, JobStateSummaryStates)
...
# Register cattrs hooks for JobStateSummaryStates
def _structure_jobstatesummarystates(data: dict[str, Any], _: type[JobStateSummaryStates]) -> JobStateSummaryStates:
    """Structure hook for cattrs to handle JobStateSummaryStates deserialization with typed values."""
    if data is None:
        return JobStateSummaryStates()
    if isinstance(data, JobStateSummaryStates):
        return data
    ...
    return JobStateSummaryStates(_data=structured_data)


def _unstructure_jobstatesummarystates(instance: JobStateSummaryStates) -> dict[str, Any]:
    """Unstructure hook for cattrs to handle JobStateSummaryStates serialization."""
...
converter.register_structure_hook(JobStateSummaryStates, _structure_jobstatesummarystates)
converter.register_unstructure_hook(JobStateSummaryStates, _unstructure_jobstatesummarystates)
