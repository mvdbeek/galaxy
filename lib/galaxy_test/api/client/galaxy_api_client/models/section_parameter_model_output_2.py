...
__all__ = ["SectionParameterModelOutput2"]


@dataclass
class SectionParameterModelOutput2:
    """
    SectionParameterModelOutput2 dataclass

    Args:
        name (str)               : Parameter name. Used when referencing parameter in
                                   workflows or inside command templating.
        parameters (SectionParameterModelOutputParameters)
                                 :
        type_ (GalaxyToolParameterModelOutputTypeEnum)
                                 : Maps from 'type'
        argument (SectionParameterModelOutputArgument | None)
        ...
        help_ (Help32 | None)    : Short bit of text, rendered on the tool form just below
                                   the associated field to provide information about the
                                   field. (maps from 'help')
        hidden (bool | None)     :
        is_dynamic (bool | None) :
        label (SectionParameterModelOutputLabel | None)
