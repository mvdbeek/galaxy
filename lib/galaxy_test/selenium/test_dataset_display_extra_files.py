...
class TestDatasetDisplayExtraFiles(SeleniumTestCase):
    ...
    def test_html_display_extra_files_relative_path(self):
        ...
        extra_file_status = self.execute_script(
            f"""
        ...
        assert extra_file_status == 200, (
            f"Extra file not accessible at {extra_file_url}, got HTTP {extra_file_status}"
        )
