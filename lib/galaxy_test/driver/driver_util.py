...
def _is_tus_route(route) -> bool:
    ...
    FastAPI keeps an included router as a single ``_IncludedRouter`` entry carrying no
    ``path`` of its own, so matching on ``route.path`` alone finds nothing and the stale
    routes survive rebinding. Fall back to the included router's own prefix.
    """
...
def caching_fast_app_factory(gx_wsgi_webapp, gx_app):
    ...
    Builds a fresh app instead whenever the launch needs a shape
    ``_rebind_fast_app_for_launch`` cannot produce: a non-default
    ``galaxy_url_prefix`` or MCP gives a parent wrapper / lifespan-bound app,
    and ``use_access_logging_middleware`` selects between two middleware sets
    that are installed once at build time and never re-bound.
    """
