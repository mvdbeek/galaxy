...
def parse_changelog(package: Package) -> List[ChangelogItem]:
    changelog = package.history_rst.read_text()
    ...
    for line in lines:
        ...
        if line.startswith("---"):
            if current_version is not None:
                current_changes = []
                changelog_item = ChangelogItem(version=current_version, date=current_date, changes=current_changes)
        ...
        if line.startswith("*"):
            ...
            continue
        if line and not current_version:
            current_date = None
            if " (" in line:
                version_str, current_date = line.split(" (")
...
def update_package_history(package: Package, new_version: Version):
    ...
    if not package.prs:
        ...
        changes.append("No recorded changes since last release")
    else:
        for pr in sorted(package.prs, key=lambda pr: pr.number):
            changes.append(f"* {pr.title} by @{pr.user.login} in {pr.html_url}")

    now = datetime.datetime.now().strftime("%Y-%m-%d")
    package.package_history.insert(0, ChangelogItem(version=new_version, changes=changes, date=now))
