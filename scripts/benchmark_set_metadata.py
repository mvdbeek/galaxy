...
def prepare_case(case_name, strategy):
    extension, contents, expected_metadata = CASES[case_name]
    case = TestMetadata()
    case.setUp()
    ...
    return case, output_dataset, sa_session, command, expected_metadata
...
def validate_result(case, output_dataset, sa_session, expected_metadata):
...
def benchmark_case(case_name, strategy, warmups, repetitions):
    case, output_dataset, sa_session, command, expected_metadata = prepare_case(case_name, strategy)
    timings = {"normal": [], "all": []}
    try:
        for _ in range(warmups):
            for mode in ("normal", "all"):
                run_once(case, command, mode)
                validate_result(case, output_dataset, sa_session, expected_metadata)
